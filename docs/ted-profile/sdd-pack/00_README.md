...
**Purpose:** Provide a *project-ingestable* spec pack to build a Mac-first OpenClaw fork ("Ted Profile") with a draft-only, auditable Microsoft Graph workflow and governed add-ons.
...
2. **Treat everything else as implementation guidance**  
   Many technical details in the source emails/docs are *user narrative* and can evolve. This pack intentionally focuses on the *job* and the *safety boundaries*.

3. **Implement as Epics + Job Cards**
   - Use `10_ROADMAP_JOB_BOARD.md` as the backlog spine.
   - Every job card includes *Proof* steps (deterministic commands/tests) and a *Definition of Done*.
