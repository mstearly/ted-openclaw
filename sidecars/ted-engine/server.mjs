...
const HOST = process.env.TED_ENGINE_HOST?.trim() || "127.0.0.1";
const PORT_RAW = Number.parseInt(process.env.TED_ENGINE_PORT || "48080", 10);
const PORT = Number.isFinite(PORT_RAW) && PORT_RAW > 0 && PORT_RAW <= 65535 ? PORT_RAW : 48080;
...
const graphProfilesConfigPath = (() => {
  const configuredPath = process.env.TED_ENGINE_GRAPH_PROFILES_PATH?.trim();
  if (!configuredPath) {
    return path.join(__dirname, "config", "graph.profiles.json");
  }
...
function getPlannerConfig(profileId) {
  ...
  return config.profiles?.[profileId]?.planner || null;
}

function getTodoConfig(profileId) {
  const config = readGraphProfilesConfig();
  return config.profiles?.[profileId]?.todo || null;
}

function getUserMapping(profileId) {
  const config = readGraphProfilesConfig();
  return config.profiles?.[profileId]?.user_mapping || {};
}

function _getSharePointConfig(profileId) {
  const config = readGraphProfilesConfig();
  return config.profiles?.[profileId]?.sharepoint || null;
}
...
function toEnvProfileKey(profileId) {
...
function getGraphRuntimeGuidOverride(profileId, fieldName) {
  const key = toEnvProfileKey(profileId);
  if (!key) {
    return "";
  }
  const envNames = [`TED_GRAPH_${key}_${fieldName}`, `TED_${key}_${fieldName}`];
...
function applyGraphProfileRuntimeOverrides(profileMap) {
  if (!profileMap || typeof profileMap !== "object") {
    return profileMap;
  }
  const merged = { ...profileMap };
  for (const [profileId, profile] of Object.entries(profileMap)) {
    if (!profile || typeof profile !== "object") {
      continue;
    }
    const tenantId = getGraphRuntimeGuidOverride(profileId, "TENANT_ID");
    const clientId = getGraphRuntimeGuidOverride(profileId, "CLIENT_ID");
    if (!tenantId && !clientId) {
      continue;
    }
    const nextProfile = { ...profile };
    if (tenantId) {
      nextProfile.tenant_id = tenantId;
    }
    if (clientId) {
      nextProfile.client_id = clientId;
    }
    merged[profileId] = nextProfile;
  }
  return merged;
}
...
function readGraphProfilesConfig() {
  try {
    ...
    return { profiles: applyGraphProfileRuntimeOverrides(parsed.profiles) };
  } catch {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ─── SharePoint Routes ───
    const sharePointHandled = await dispatchSharePointRoute(
      { method, route, parsed, req, res },
      {
        isSlugSafe,
        sendJson,
